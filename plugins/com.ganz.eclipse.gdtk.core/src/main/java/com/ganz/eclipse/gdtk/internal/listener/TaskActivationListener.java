...
public class TaskActivationListener implements ITaskActivationListener {
	...
	private class RestartWorkbenchJob extends UIJob {
		...
		/**
		 * Restart the workbench using the specified path as the workspace
		 * location.
		 *
		 * @param path
		 *            the location
		...
		private void restart(String path) {
			...
			//
			// String command_line = "\"C:\\Program
			// Files\\Java\\jdk1.8.0_71\\bin\\javaw.exe\"
			// -agentlib:jdwp=transport=dt_socket,suspend=y,address=localhost:54920
			// -Declipse.pde.launch=true -Dfile.encoding=Cp1252 -classpath
			// \"C:\\work\\tool\\eclipse\\.p2\\pool\\plugins\\org.eclipse.equinox.launcher_1.3.201.v20161025-1711.jar\"
			// org.eclipse.equinox.launcher.Main -launcher
			// \"C:\\work\\ganz\\tool\\eclipse\\devel\\neon\\eclipse.exe\" -name
			// Eclipse -showsplash 600 -product org.eclipse.platform.ide -data
			// \"C:\\work\\ganz\\proj\\releng\\eclipse\\master\\..\\runtime\\studio\"
			// -configuration
			// file:C:/work/ganz/proj/releng/eclipse/master/.metadata/.plugins/org.eclipse.pde.core/Studio/
			// -dev
			// file:C:/work/ganz/proj/releng/eclipse/master/.metadata/.plugins/org.eclipse.pde.core/Studio/dev.properties
			// -os win32 -ws win32 -arch x86_64 -nl en_US -consoleLog -debug";
			//
		...
		/**
		 * Create and return a string with command line options for eclipse.exe
		 * that will launch a new workbench that is the same as the currently
		 * running one, but using the argument directory as its workspace.
		 *
		 * @param workspace
		...
		private String buildCommandLine(String workspace) {
			...
			// append the rest of the args, replacing or adding -data as
			// required
			...
			// put the vmargs back at the very end (the eclipse.commands
			// property
	...
	public void taskActivated(ITask task) {
		// TODO Auto-generated method stub
		this.task = task;
		// Logger.getInstance().info(String.format("Activating %s task",
		// task.getTaskKey()));
	...
	public void taskDeactivated(ITask task) {
		// TODO Auto-generated method stub
		this.task = task;
		// Logger.getInstance().info(String.format("Deactivate %s task",
		// task.getTaskKey()));
	}
